...
(async () => {
  ...
  const predict = ({ stream, slideStep, maxZoomExp }) => {
    ...
    for (const index in stream) {
      ...
      while (zoom <= max_zoom) {
        ...
        for (let i = 0; i <= overflow_x; i += step) {
          for (let j = 0; j <= overflow_y; j += step) {
            ...
            pendingBuffer.push(createImageBitmap(copied, i, j, width, height).then(slicedImageBitmap => {
              ...
              return tf.tidy(() => tf.browser.fromPixels(sliced).mean(2).toFloat().expandDims(-1).div(divider).reshape([1, 128, 64, 1]));
            }));
          }
        }
        zoom *= ZOOM_MULT;
      ...
      pendingOutput.push(Promise.all(pendingBuffer).then(buffer => {
    ...
    return Promise.all(pendingOutput).then(output => {
      divider.dispose();
      return output;
    });
  }
